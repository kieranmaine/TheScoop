...
function createComment(url, request) {
  ...
  if (!(database.users[requestComment.username] && database.articles[requestComment.articleId]))
  {
    response.status = 400;
    return response;
  }
...
function deleteComment(url, request) {
  ...
  const articleComments = database.articles[comment.articleId].commentIds;
  articleComments.splice(x => x === articleComments.findIndex(id), 1);

  const userComments = database.users[comment.username].commentIds;
  userComments.splice(x => x === userComments.findIndex(comment.id), 1);
...
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);  
  const username = request && request.body && request.body.username;
  const response = {};

  const comment = database.comments[id];

  if (!(comment && username && database.users[username])) {
    response.status = 400;
    return response;    
  }
  
  return updateVotedByAndReturnResponse(comment, username, "upvote");
}
...
function downvoteComment(url, request) {
  ...
  return updateVotedByAndReturnResponse(comment, username, "downvote");
}

function updateVotedByAndReturnResponse(comment, username, voteType) {
  const addToArr = voteType === "upvote" ? comment.upvotedBy : comment.downvotedBy;
  const removeFromArr = voteType === "upvote" ? comment.downvotedBy : comment.upvotedBy;

  if (!addToArr.includes(username)) {
    addToArr.push(username);

    const index = removeFromArr.findIndex(x => x === username);
    if (index > -1)
      removeFromArr.splice(index, 1);
  }
...
function saveDatabase() {
  const yamlString = yaml.safeDump(database);
  fs.writeFileSync('./database.yml', yamlString, 'utf8');
}
